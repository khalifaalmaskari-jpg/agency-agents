...
1. **Route — adopt the Front Desk Router.**
   ...
   - Ask at most 2 clarifying questions, and only when the answer changes WHO handles it.
   - If the directive matches a dedicated skill (/collect, /campaign, /hire, /tender, /bia, /market-entry, /legal-prep, /crisis, /payroll, /reputation, /incident, /it-landscape, /succession), say so and run that skill's flow instead of re-deriving it.
...
2. **Input request** — the primary owner names the specific data its deliverable needs (exports, aging lists, balances, drafts); collect what the user has; everything else becomes `[DATA NEEDED]` placeholders, never invented.
...
4. **Adopt the owning chief (department-scale directives).** For directives that span a department or several specialists, adopt the chief who owns that department between routing and specialist execution: the chief assigns the work, and — once the specialists have produced — runs the FIRST quality pass (sources present, assumptions tagged, on-brief) before the gate. Single-specialist asks may skip the chief; say so when skipping.

5. **Execute — adopt each specialist in sequence.**
...
6. **Gate — adopt the Revalidation Gatekeeper.** Run the five checks, in order, on the combined work:
...
7. **Fix cycle (on RETURN).**
   ...
   - Re-gate (Step 6). Maximum 2 return cycles.
...
- **The gate is not optional.** Nothing reaches the user without Step 6's verdict. There is no "pass with concerns" and no skipping the gate for small jobs.
